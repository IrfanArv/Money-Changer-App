	...
	public function index()
	{
		...
		if(! $id){
			$this->tdk_ada_sesion();
				...
				$this->layout->display('valas/admin/AdministratorV', $data);  
			}elseif($this->session->userdata('level')=='2'){
				$data['title']=$title = 'Backoffice Bafageh Money Changer';
				$this->layout->display('valas/BackofficeV', $data);
			}elseif($this->session->userdata('level')=='3'){
				$data['title']=$title = 'Teller Bafageh Money Changer';
				$this->layout->display('valas/TellerV', $data);
			}elseif($this->session->userdata('level')=='4'){
				$data['title']=$title = 'General Manager Bafageh Money Changer';
				$this->layout->display('monitoring/GemanV', $data);
			}
	}
