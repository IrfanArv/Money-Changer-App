            ...
            <div class="page-title-box">
                <div class="float-right">
                     <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Pembelian Hari ini</li>
                    </ol> 
                </div>
                
                <h4 class="page-title">Pembelian Hari ini</h4>
            </div>
        </div>
    </div>
...
<script type="text/javascript"> 
    ...
    $(document).ready(function() {
        table = $('#table').DataTable({ 
            ...
            "order": [], 
            "ajax": {
                "url": "<?php echo site_url('valas/adminvalas/pembelian_list')?>",
                "type": "POST"
            },
    ...
    { 
        ...
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url : "<?php echo site_url('valas/adminvalas/batalin_transaksi')?>/"+id,
                        type: "POST",
                        dataType: "JSON",
    ...
    function sahkantransaksi(id)
    {
        ...
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url : "<?php echo site_url('valas/adminvalas/sahkantransaksi')?>/"+id,
                        type: "POST",
                        dataType: "JSON",
                        success:function(result){
                            if(result.status == true){
    ...
    function terimatransaksi(id)
    {
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url : "<?php echo site_url('valas/adminvalas/terimatransaksi')?>/"+id,
                    type: "POST",
                    dataType: "JSON",
                    success:function(result){
                        if(result.status == true){
