...
<script type="text/javascript"> 
    ...
    // TABLE WAITING LIST 
        $(document).ready(function() {
            ...
            table = $('#tablewaiting').DataTable({ 
                ...
                "ajax": {
                    "url": "<?php echo site_url('valas/backoffice/waiting_list')?>",
                    "type": "POST"
                },
    ...
    // RELOAD
        function reload_table()
        {
            ...
            $( "#total_barang" ).load( "<?php echo site_url('valas/backoffice/jumlah_barang_user')?>" );
            $( "#transaksi_beli_valid" ).load( "<?php echo site_url('valas/backoffice/hitung_sukses_beli')?>" );
            $( "#transaksi_beli_gagal" ).load( "<?php echo site_url('valas/backoffice/hitung_gagal_beli')?>" );
    ...
    // MODALVALIDASI
        ...
        {
            $.ajax({
                url:"<?php echo site_url('valas/backoffice/validasibeli/')?>/"+ no_invoice,
                method:"POST",
    ...
    // HITUNG PEMBELIAN SELESAI BY USERBACKOFFICE
        $(document).ready(function() {
            $.ajax({    
                type: "GET",
                "url": "<?php echo site_url('valas/backoffice/hitung_sukses_beli')?>",
                dataType: "html",   
    ...
    // HITUNG PEMBELIAN DI TOLAK BY USER BACKOFFICE
        $(document).ready(function() {
            $.ajax({    
                type: "GET",
                "url": "<?php echo site_url('valas/backoffice/hitung_gagal_beli')?>",
                dataType: "html",   
                success: function(response){                    
    ...
    // ACC BELI
        function accpembelian(no_invoice)
        {
                ...
                if (isConfirm) { 
                    $.ajax({
                        url : "<?php echo site_url('valas/backoffice/accbeli/')?>/"+ no_invoice,
                        type: "post",
                        dataType:"json",
                        success:function(result){
                            if(result.status == true){
                                ...
                                $('.res').val('');
                                $( "#transaksi_beli_valid" ).load( "<?php echo site_url('valas/backoffice/hitung_sukses_beli')?>" );
                                $( "#total_acc_beli" ).load( "<?php echo site_url('valas/backoffice/jumlah_sukses_beli')?>" );
                                $( "#isiModalbelisukses" ).load( "<?php echo site_url('valas/backoffice/detail_sukses_beli')?>" );
    ...
    // TOLAK BELI
        ...
        function tolakpembelian(no_invoice) 
        {
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url : "<?php echo site_url('valas/backoffice/tolakbeli/')?>/"+ no_invoice,
                        type: "post",
                        dataType:"json",
                        success:function(result){
                            if(result.status == true){
                                ...
                                reload_table();
                                $('.res').val('');
                                $( "#isiModalbelisukses" ).load( "<?php echo site_url('valas/backoffice/detail_tolak_beli')?>" );
                                $( "#total_tolak_beli" ).load( "<?php echo site_url('valas/backoffice/jumlah_tolak_beli')?>" );
                                $( "#transaksi_beli_gagal" ).load( "<?php echo site_url('valas/backoffice/hitung_gagal_beli')?>" );
    ...
    //TOTAL HARGA JUMLAH BELI 
        $(document).ready(function() {
                ...
                $.ajax({    
                    type: "GET",
                    "url": "<?php echo site_url('valas/backoffice/jumlah_sukses_beli')?>",
                    dataType: "html",   
                    success: function(response){                    
    ...
    // TOTAL HARGA JUMLAH DITOLAK
        $(document).ready(function() {
            $.ajax({    
                type: "GET",
                "url": "<?php echo site_url('valas/backoffice/jumlah_tolak_beli')?>",
                dataType: "html",   
                success: function(response){                    
    ...
    // LIST DETAIL RECAP PEMBELIAN SELESAI
        ...
        {
            $.ajax({
                url:"<?php echo site_url('valas/backoffice/detail_sukses_beli/');?>",
                method:"POST",
                success:function(data){
    ...
    // LIST DETAIL RECAP PEMBELIAN DI TOLAK
        ...
        {
            $.ajax({
                url:"<?php echo site_url('valas/backoffice/detail_tolak_beli/');?>",
                method:"POST",
                success:function(data){
                    $('#modal_recap_beli_success').modal('show'); 
    ...
    function detailriwayat(no_invoice)
        {
            $.ajax({
                url:"<?php echo site_url('valas/backoffice/detailriwayatbackoffice/')?>/"+ no_invoice,
                method:"POST",
                success:function(data){
    ...
    function laci() 
        { 
            $.ajax({ 
                url:"<?php echo site_url('valas/backoffice/laci_kamu/');?>",
                method:"POST",
                success:function(data){
                    $('#modal_laci').modal('show'); 
                    $('.modal-title').text('Stock Global Realtime'); 
                    $('#isilaci').html(data);
                    }
                });
        }
    $(document).ready(function() {
        ...
        $.ajax({    
            type: "GET",
            "url": "<?php echo site_url('valas/backoffice/jumlah_barang_user')?>",
            dataType: "html",    
            success: function(response){                    
                $("#total_barang").html(response);   
            }
        });
    }); 
